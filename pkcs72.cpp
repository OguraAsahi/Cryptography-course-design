...
EVP_PKEY* getpkey(const char* private_key)                           //获取私钥
{
...
void pkcs7decrypt2(unsigned char*testbuf,int testlen)
{
	...
	//BIO_read_filename(bio_data, datafile);
	...
	if (len <= 0)                                       //判断是否解密失败，如果失败则输出ERROR
	{
		printf("ERROR");
		return;
	}
...
int main()
{
	freopen("D:\\gmsslfile\\1.pem", "r",stdin);
	char linetemp[1010];                                       //读入的每一行数据
	while (!cin.eof())
	{
...
bool verifyCRT(char* varbuf,unsigned int varlen)                        //验证证书
{
	...
	EVP_PKEY* pubkey = X509_get_pubkey(test);               //从CA证书中获取公钥
	/*test2 = der ?
		d2i_PKCS7_bio(crttest, NULL):*/
	if (test2 == NULL)
		return 0;
...
bool verifySign(char* signature_msg,unsigned int length)
{
	...
	for (int i = 0; i < signcount; i++)
	{
		...
		STACK_OF(X509)* certs = NULL;
		certs = p7->d.signed_and_enveloped->cert;                        //取出证书列表
...
bool ifcansign(X509*test)                   //要检查能否进行签名
{
	ASN1_BIT_STRING* test2;
	unsigned short superid;
	int cmp = X509_get_ext_by_NID(test, NID_key_usage, 0);                //要获取数字签名的类型
